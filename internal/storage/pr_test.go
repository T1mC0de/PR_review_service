	...
	for _, c := range cases {
		 t.Run(c.name, func(t *testing.T){
			 ...
			 case "create":
				 ...
				 if (err != nil) != c.wantErr { t.Errorf("create err=%v wantErr=%v", err, c.wantErr) }
			 case "merge":
				 err := stor.MergePullRequest(ctx, c.prID)
				 if (err != nil) != c.wantErr { t.Errorf("merge err=%v wantErr=%v", err, c.wantErr) }
			 case "reassign":
				 _, err := stor.ReassignPullRequest(ctx, c.prID, c.oldUserID)
				 if (err != nil) != c.wantErr { t.Errorf("reassign err=%v wantErr=%v", err, c.wantErr) }
			 }
		 })
	}
