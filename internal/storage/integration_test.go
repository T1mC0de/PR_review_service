...
// ensureStorage opens real DB (requires docker postgres up)
...
func TestTeamAndUsersIntegration(t *testing.T) {
	...
	members := []models.DBUser{{UserID: uniqueName("usr"), Username: "UserA", IsActive: true, CreatedAt: time.Now()}, {UserID: uniqueName("usr"), Username: "UserB", IsActive: true, CreatedAt: time.Now()}}
...
func TestPullRequestLifecycleIntegration(t *testing.T) {
	...
	rev1 := uniqueName("rev")
	rev2 := uniqueName("rev")
	members := []models.DBUser{{UserID: author, Username: author, IsActive: true, CreatedAt: time.Now()}, {UserID: rev1, Username: rev1, IsActive: true, CreatedAt: time.Now()}, {UserID: rev2, Username: rev2, IsActive: true, CreatedAt: time.Now()}}
	if err := stor.CreateTeam(ctx, team, members); err != nil {
		t.Fatalf("CreateTeam err: %v", err)
	}
...
func TestMassDeactivateAndStatsIntegration(t *testing.T) {
	...
	users := []models.DBUser{{UserID: uniqueName("u"), Username: "A", IsActive: true, CreatedAt: time.Now()}, {UserID: uniqueName("u"), Username: "B", IsActive: true, CreatedAt: time.Now()}, {UserID: uniqueName("u"), Username: "C", IsActive: true, CreatedAt: time.Now()}}
	if err := stor.CreateTeam(ctx, team, users); err != nil {
		t.Fatalf("CreateTeam err: %v", err)
	}

	// create PR by first user
	prID := uniqueName("mpr")
	if err := stor.CreatePullRequest(ctx, prID, "Title", users[0].UserID); err != nil {
		t.Fatalf("CreatePullRequest err: %v", err)
	}

	// Mass deactivate two users (including author) to test branch where author deactivated
